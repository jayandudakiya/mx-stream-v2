 ...
 * overlay). Controlled from Dart via `orcabox/exoplayer_<id>` (setSource /
 * play / pause / seekTo) and streams playback state on
 * `orcabox/exoplayer_events_<id>`.
 *
...
class ExoPlayerView(
    ...
    private val channel = MethodChannel(messenger, "orcabox/exoplayer_$id")
    private val events = EventChannel(messenger, "orcabox/exoplayer_events_$id")
