...
import com.orcabox.app.aniyomi.AniyomiBridge
import com.orcabox.app.cast.CastManager
import com.orcabox.app.cloudstream.CsPluginLoaderActivity
import com.orcabox.app.cloudstream.PluginHost
import com.orcabox.app.cloudstream.RepoManager
import com.orcabox.app.cloudstream.SubscriptionWorker
import com.orcabox.app.mihon.MihonBridge
...
/// Hosts the "orcabox/seek_preview" channel, which decodes a single video
...
class MainActivity : AppCompatActivity(), FlutterEngineConfigurator {
    ...
    private val channelName = "orcabox/seek_preview"
    ...
    private val ICON_ALIASES = linkedMapOf(
        "default" to "com.orcabox.app.MainActivityDefault",
        "classic" to "com.orcabox.app.MainActivityClassic",
    )
    ...
    /// android:enabled, which is true only for the Default (full-colour
    /// OrcaBox) alias — so that is what an untouched install is really
    /// showing. Must match `AppIconService.defaultId` and the manifest.
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/external_player")
            .setMethodCallHandler { call, result ->
                when (call.method) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/download")
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "remuxTsToMp4" -> {
                        ...
                        if (input.isNullOrEmpty() || output.isNullOrEmpty()) {
                            ...
                            Thread {
                                val ok = com.orcabox.app.download.TsRemuxer.remux(input, output)
        ...
        tvBridge = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/tv_player")
            .also { ch ->
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/app_icon")
            .setMethodCallHandler { call, result ->
                when (call.method) {
        ...
        val pipChannel =
            MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/pip")
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/voice_search")
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "isAvailable" -> {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/cloudstream")
            .also { csChannel = it }
            .setMethodCallHandler { call, result ->
                when (call.method) {
                            ...
                            try {
                                if (repoUrl.isNotEmpty()) {
                                    host.deleteByRepoPlugin(
                                        com.orcabox.app.cloudstream.RepoManager.repoTag(repoUrl),
                                        internalName,
                                    )
                                } else {
                    ...
                    "updateRepo" -> {
                        ...
                        csExecutor.execute {
                            try {
                                ...
                                // same-named plugin.
                                val tag = com.orcabox.app.cloudstream.RepoManager
                                    .repoTag(url ?: "")
                        ...
                        runOnUiThread {
                            try {
                                val intent = android.content.Intent(
                                    this,
                                    com.orcabox.app.cloudstream.CloudStreamSettingsActivity::class.java,
                                ).putExtra(
                                    com.orcabox.app.cloudstream.CloudStreamSettingsActivity.EXTRA_API_NAME,
                                    name,
                                )
        ...
        val aniyomiChannel = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/aniyomi")
        ...
        val mihonChannel = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/mihon")
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/novel_http")
            .setMethodCallHandler { call, result ->
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "com.orcabox.app/device")
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "isTv" -> {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/cast")
            .setMethodCallHandler { call, result ->
                try {
                    when (call.method) {
        ...
        EventChannel(flutterEngine.dartExecutor.binaryMessenger, "orcabox/cast/events")
            .setStreamHandler(cm)
    }
    ...
    private fun mergeSubscriptions(incoming: List<Map<String, Any?>>) {
        val prefs = getSharedPreferences("orcabox_cs", android.content.Context.MODE_PRIVATE)
    ...
    private fun scheduleSubscriptionWorker(hasSubs: Boolean) {
        ...
        if (!hasSubs) {
            wm.cancelUniqueWork("orcabox_sub_check")
            return
        }
