...
abstract class AnimeHttpSource : AnimeCatalogueSource {
     ...
     *
     * Default: returns [listOf(anime)] — i.e. no multi-season navigation; the caller goes
     * straight to [getEpisodeList]. This matches mainstream Aniyomi v16 behavior where most
     * extensions do not implement seasons and the default resolves to the single anime itself.
     *
     * Override this AND [seasonListRequest]/[seasonListParse] when a source genuinely exposes
     * multiple seasons. Calling super.getSeasonList() is NOT safe if seasons are expected —
     * subclasses that support seasons should fully replace this method.
     *
     * @since extensions-lib 16
     * @param anime the anime to look for seasons.
     * @return the seasons for the anime; default is [listOf(anime)].
     */
    override suspend fun getSeasonList(anime: SAnime): List<SAnime> = listOf(anime)
     ...
     *
     * Default returns an empty list. Sources that override [getSeasonList] to perform an HTTP
     * request should also override this method to parse the response. Keeping this non-abstract
     * is required for mainstream v16 compatibility — extensions that do not support seasons must
     * not be forced to implement this (doing so would cause AbstractMethodError).
     *
     * @since extensions-lib 16
     * @param response the response from the site.
     * @return the list of seasons; default is empty.
     */
     ...
     *
     * Default returns an empty list. Sources that override [getHosterList] to perform an HTTP
     * request should also override this. Non-abstract for mainstream v16 compatibility —
     * extensions that do not use the Hoster API must not be forced to implement this.
     *
     * @since extensions-lib 16
     * @param response the response from the site.
     * @return the list of hosters; default is empty.
     */
     ...
     *
     * Default returns an empty list. Sources using the Hoster API (lib-16) should override
     * this together with [videoListRequest(hoster)]. Non-abstract for mainstream v16 compatibility
     * — extensions that use the legacy [videoListParse(Response)] path do not implement this.
     *
     * @since extensions-lib 16
     * @param response the response from the hoster.
     * @param hoster the hoster.
     * @return the list of videos; default is empty.
     */
