...
/// Base type for all Mihon filter elements.
///
/// The 8 subtypes mirror the native `Filter` class hierarchy (manga side).
/// Verified against `MihonFilterJson.kt`: it produces the exact same JSON
/// shapes as `AniyomiFilterJson.kt` (same type strings, same extra-field
/// sets, same Sort/TriState conventions) — this is a clean rename twin of
/// `aniyomi_filters.dart`, not a re-derivation. The Dart side parses a schema
/// JSON string emitted by the native bridge, the UI sheet mutates the leaf
/// states in place, and [MihonFilters.toSelectionJson] re-serialises the list
/// so the native side can apply selections back by position.
sealed class MihonFilter {
  const MihonFilter({required this.name});
...
final class MihonHeader extends MihonFilter {
  const MihonHeader({required super.name});
}

/// A visual divider between filter sections.
final class MihonSeparator extends MihonFilter {
  const MihonSeparator({required super.name});
}
...
final class MihonSelect extends MihonFilter {
  MihonSelect({
    required super.name,
...
final class MihonText extends MihonFilter {
  MihonText({required super.name, required this.state});
...
final class MihonCheckBox extends MihonFilter {
  MihonCheckBox({required super.name, required this.state});

  /// Whether the checkbox is checked. Updated by the filter sheet.
  bool state;
}

/// A three-state toggle mirroring `Filter.TriState`:
/// - 0 = ignore
/// - 1 = include
/// - 2 = exclude
///
/// [state] is mutable so the filter sheet can cycle through the values.
final class MihonTriState extends MihonFilter {
  MihonTriState({required super.name, required this.state});
...
/// A named group of child filters (typically [MihonCheckBox] or
/// [MihonTriState] entries).
final class MihonGroup extends MihonFilter {
  MihonGroup({required super.name, required this.children});
...
final class MihonSort extends MihonFilter {
  MihonSort({
    required super.name,
    required this.values,
    required this.index,
...
class MihonFilters {
  ...
  static List<MihonFilter> parse(String json) {
    try {
  ...
  static List<MihonFilter> _parseList(List<dynamic> raw) {
    final result = <MihonFilter>[];
  ...
  static MihonFilter? _parseElement(Map<String, dynamic> e) {
    final String? type = e['type'] as String?;
    final String name = (e['name'] as String?) ?? '';

    switch (type) {
      case 'header':
        return MihonHeader(name: name);

      case 'separator':
        return MihonSeparator(name: name);
      ...
      case 'checkbox':
        return MihonCheckBox(
          name: name,
          state: (e['state'] as bool?) ?? false,
        );

      case 'tristate':
        return MihonTriState(
          name: name,
      ...
      case 'group':
        ...
        return MihonGroup(name: name, children: children);
  ...
  static String toSelectionJson(List<MihonFilter> list) {
    return jsonEncode(list.map(_encode).toList());
  }

  static Map<String, dynamic> _encode(MihonFilter f) {
    return switch (f) {
      MihonHeader() => {'type': 'header', 'name': f.name},
      MihonSeparator() => {'type': 'separator', 'name': f.name},
      MihonSelect() => {
          'type': 'select',
          'name': f.name,
          'values': f.values,
