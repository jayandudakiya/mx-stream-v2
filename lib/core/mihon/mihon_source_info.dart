...
///
/// Field names match the JSON keys emitted by `MihonBridge.sourcesJson()`:
/// `id`, `name`, `lang`, `nsfw`, `pkg`, `baseUrl`, `headers`, `version`,
/// `versionCode` — the same 9 keys `AniyomiBridge.sourcesJson()` emits for
/// the anime path (verified against the Kotlin; no key added, dropped, or
/// renamed).
class MihonSourceInfo {
  const MihonSourceInfo({
    required this.id,
    required this.name,
    required this.lang,
  ...
  /// Numeric source id matching `Source.id` on the native side.
  final int id;
  final String name;
  final String lang;
  ...
  ///
  /// Required by some image hosts to serve covers without 403 errors.
  ...
  factory MihonSourceInfo.fromJson(Map<String, dynamic> json) {
    return MihonSourceInfo(
