...
class SourceRepository implements CatalogueRepository {
  SourceRepository({
    ...
    NativeProviderManager? nativeManager,
    ...
    AnimeLangPrefs? animeLangs,
       ...
       // Native movie providers (VegaMovies/RogMovies) are pre-installed and
       // have no external dependency to wait on, so — unlike Mihon/LNReader —
       // there's no reason to default this to an empty registry; every caller
       // (real app and tests alike) gets Hollywood/Bollywood browsing for free.
       _nativeManager = nativeManager ?? NativeProviderManager();
  ...
  final NativeProviderManager _nativeManager;
  ...
  String displayName(String sourceId) {
    ...
    if (_isNative(sourceId)) {
      return _nativeManager.get(sourceId)?.displayName ?? sourceId;
    }
  ...
  bool hasSource(String sourceId) {
    ...
    if (_isNative(sourceId)) {
      return _nativeManager.get(sourceId) != null;
    }
  ...
  BaseProvider _providerFor(String? id) {
    ...
    } else if (_isNative(resolved)) {
      p = _nativeManager.get(resolved);
    } else {
