...
// The OrcaBox (JS-provider) "Add repo" dialog used to offer a recommended
...
class _FakeManifestAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    final body = jsonEncode({
      'name': 'Recommended Sozo Providers',
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('orcabox_add_repo_test');
  ...
  Future<void> openAddRepoDialog(WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: OrcaBoxSourcesScreen()));
    await tester.pump();
    await tester.tap(find.text('Add OrcaBox repo'));
    await tester.pump();
